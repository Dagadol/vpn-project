...
nat = nat_class.ClassNAT(my_ip="10.0.0.20", users=vpn_server.clients)
clients = {"10.0.0.50": ("10.0.0.11", 8800)}  # virtual adapter: (skt.ip, skt.port)


def add_user(addr):
    if addr[0] not in nat.users_addr:
        nat.users_addr[addr[0]] = addr[1]


def main():
    skt = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    skt.bind(("0.0.0.0", vpn_server.udp_port))

    t_recv = threading.Thread(target=internet_recv, args=[skt], daemon=True)
    t_send = threading.Thread(target=internet_send, args=[skt], daemon=True)
    t_recv.start()
    t_send.start()
    t_recv.join()
    t_send.join()
...
def internet_send(skt):
    while True:
        ...
        if addr[0] not in vpn_server.keys:
            continue
        pkt = valid(data)
        if pkt:
